...
class Up(nn.Module):
   def __init__(self,in_channels,out_channels,bilinear=True,dropout_prob=0.0):
      ...
      if bilinear:
         ...
         self.conv = doubleConv(in_channels,out_channels,dropout_prob,in_channels//2) # 拼接后为1024，经历第一个卷积后512
      else:
         # 转置卷积实现上采样
         # 输出通道数减半，宽高增加一倍
...
# 双卷积块
def doubleConv(in_channels,out_channels,dropout_prob,mid_channels=None):
   '''
   :param in_channels: 输入通道数 
   ...
   if dropout_prob > 0:
        # 使用 nn.Dropout2d 更适合卷积层
        layer.append(nn.Dropout2d(p=dropout_prob))
...
# 下采样
def down(in_channels,out_channels,dropout_prob):
   # 池化 + 双卷积
   layer = []
   layer.append(nn.MaxPool2d(2,stride=2))
   layer.append(doubleConv(in_channels,out_channels,dropout_prob))
   return nn.Sequential(*layer)
...
class U_net(nn.Module):
   def __init__(self,in_channels,out_channels,bilinear=True,base_channel=64,dropout_prob=0.0):
      ...
      # 输入
      self.in_conv = doubleConv(self.in_channels,base_channel,dropout_prob)
      # 下采样
      self.down1 = down(base_channel,base_channel*2,dropout_prob) # 64,128
      self.down2 = down(base_channel*2,base_channel*4,dropout_prob) # 128,256
      self.down3 = down(base_channel*4,base_channel*8,dropout_prob) # 256,512
      # 最后一个下采样，通道数不翻倍（因为双线性差值，不会改变通道数的，为了可以简单拼接，就不改变通道数）
      # 当然，是否采取双线新差值，还是由我们自己决定
      factor = 2  if self.bilinear else 1
      self.down4 = down(base_channel*8,base_channel*16 // factor,dropout_prob) # 512,512
      # 上采样 + 拼接
